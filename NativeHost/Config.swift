...
///
/// Decoding is hand-written on purpose: Swift's synthesized `init(from:)` calls
/// `decode`, not `decodeIfPresent`, so a property default is NOT a fallback for
/// a missing key — it throws, and one absent key would reset every setting.
/// `decodeIfPresent` keeps each key independently optional.

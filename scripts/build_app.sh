#!/usr/bin/env bash
# Build a shippable "Llama Menu.app" macOS bundle.
set -euo pipefail
...
# --- Native Swift binary ----------------------------------------------------
# The CFBundleExecutable MUST be a real Mach-O binary owned by this app: a
# script host reports the interpreter's identity to macOS, and status items
# then often never appear.
echo "==> Compiling native Swift host…"
# main.swift last: top-level code is only legal in a file with that name.
swiftc -O \

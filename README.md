...
Drop GGUF files under `~/models` — subfolders become groups in the menu. Vision
models with a sibling `mmproj*.gguf` show a 👁 marker and get `--mmproj` passed
automatically.
...
The app icon, the hero above and the state strip are all generated from
`resources/menu-llama-*.svg` by `./scripts/make_assets.sh`, so none of them can
drift from what the app actually ships. All artwork here is original to this
project.
...
<p align="center">
  <sub>The app icon, as it appears in Finder and the Dock</sub>
</p>
...
- **One native binary** — a real Mach-O host with no runtime dependencies
...
├── resources/               # Glyphs, icon, launch.html + settings.html
├── docs/assets/             # README images
├── scripts/
│   ├── build_app.sh         # Compile Swift → dist/Llama Menu.app
│   ├── test.sh              # Run Tests/
│   ├── make_assets.sh       # Regenerate icon + README art from the glyphs
│   ├── install.sh
│   └── uninstall.sh
└── VERSION
```
